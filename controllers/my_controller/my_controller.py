    ...
    def run(self):
        ...
        while self.step(self.time_step) != -1:
            ...
            if self.landing:
                ...
                print("TEST!")
                led_state = 1
                self.led0.set(led_state)
                self.led1.set(led_state)
                motors = [self.front_left_motor, self.front_right_motor,
                        self.rear_left_motor, self.rear_right_motor]
                for motor in motors:
                    speed = 0.1
                    motor.setVelocity(speed)
                time_flag = self.getTime()
                counter = 0.1
                if self.getTime() < time_flag + counter and t1+7 < self.getTime() :
                    counter+=0.1
                    self.target_altitude -=0.01
                    if self.target_altitude == 0.2 :
                        break
                    
            if self.landing and altitude < 0.01:
